...
class PartialConv2d(nn.Conv2d):
    ...
    def forward(self, input, mask_in=None):  # type: ignore
        ...
        raw_out = super().forward(
            torch.mul(input, mask) if mask_in is not None else input  # type: ignore
        )

        if self.bias is not None:
            bias_view = self.bias.view(1, self.out_channels, 1, 1)
            output = torch.mul(raw_out - bias_view, self.mask_ratio) + bias_view  # type: ignore
            output = torch.mul(output, self.update_mask)  # type: ignore
        else:
            output = torch.mul(raw_out, self.mask_ratio)  # type: ignore
